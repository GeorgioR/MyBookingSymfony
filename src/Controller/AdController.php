...
{
    /**
     * Permets d'afficher une liste d'annonce
     * @Route("/ads", name="ads_list")
     */
    public function index(AdRepository $repo)
    {
        
        // via $repo on cherche toutes les annonces via la méthode findAll 
        ...
        {
         //fabriquant de formulaire de symfony: FORMBUILDER
             $ad = new Ad();
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                //Si le formulaire est soumis et si le formulaire est valide, on demande à doctrine de sauvegarder ces données dans la bdd (dans l'objet manager)
                
                //pour chaque image supplémentaire ajoutée
                foreach($ad->getImages() as $image){

                    //on relie l'image a l'nnonce et on modifie l'annonce
                    $image->setAd($ad);
                ...
                $this->addFlash('success',"Votre annonce {$ad->getTitle()} a bien été créée avec succès");
    ...
    /**
     * Permets d'afficher une seule annonce
    ...
    public function show($slug,Ad $ad)
    {
        ...
        //X = un champ de la table, à préciser
     ...
     * @Security("is_granted('ROLE_USER') and user === ad.getAuthor()", message="Vous n'êtes pas l'auteur de cette annonce pour pouvoir la supprimer")
     * @param Ad $ad
